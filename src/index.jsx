...
class PaymentIcon extends React.Component {
	...
	componentDidMount() {
		import(`${getPaymentImage(this.props.icon, this.props.transparent)}`).then(
			img => {
				this.setState({
					img: img
				});
	...
	render() {
		return (
			<React.Fragment>
				...
				>
					{this.state.img
						? React.createElement(this.state.img.default, {
								style: this.props.style
						  })
						: ""}
				</div>
...
// const PaymentIcon = ({ style, className, icon, transparent }) => {
